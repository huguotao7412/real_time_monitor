...
CFAR_SNR_UPDATE_RATIO = 2.0         # 目标更新判定：新位置SNR需高于旧位置的倍率

PHASE_RANGE_MIN_NORMAL = 0.005      # 常规模式最小有效相位极差 (低于此值判定为无人/微弱信号)
BREATH_RATIO_MIN = 0.03             # 呼吸能量占比极小阈值 (判定信号有效性)
CFAR_NEW_TARGET_MIN_SNR = 12.0      # 目标跳变时，新位置必须达到的最低 SNR
TARGET_EMPTY_SNR_THRESHOLD = 10.0   # 判定为空床(目标丢失)的近期能量 SNR 下限
PHASE_JUMP_MA_THRESHOLD = 0.8       # 体动伪影(MA)检测的相邻帧相位突变阈值 (rad)
...
HAMPEL_WINDOW_SIZE = 5              # 局部异常点剔除(Hampel滤波)的中值滤波窗长
HAMPEL_SIGMA = 3.0                  # 局部异常点剔除的容忍度乘数(MAD倍率)
EMD_RESP_CORR_RATIO = 0.7           # EMD提取呼吸主成分时的基准相关性乘数要求

# 2.2 平滑器与追踪器配置
BREATH_USE_NEW_SMOOTHER = True
HEART_USE_NEW_SMOOTHER = True

BREATH_RAW_HISTORY_MAXLEN = 4       # 中值预滤波原始队列长度
BREATH_HISTORY_MAXLEN = 5           # 呼吸 Kalman 追踪历史长度
HEART_KALMAN_HISTORY_MAXLEN = 8    # 心跳 Kalman 平滑器的历史记录截取长度

BPM_KALMAN_Q = 1e-3                 # 过程噪声协方差
BPM_KALMAN_R = 0.1                  # 观测噪声协方差
BPM_EMA_ALPHA_MIN = 0.005           # 基于 SQI 调整的最小平滑权重
BPM_EMA_ALPHA_MAX = 0.05            # 最大平滑权重
BPM_JUMP_THRESHOLD = 20.0           # 生理跳变判决阈值 (BPM)
BPM_JUMP_HOLD_COUNT = 3             # 检测到跳变时的最大 Hold 帧数

HEART_BPM_JUMP_LIMIT = BPM_JUMP_THRESHOLD         # 判定为心率异常跳变的绝对阈值 (BPM)
HEART_PROM_MIN_VALID = 0.3          # 发生疑似心率跳变时，容忍跳变所需的最低频率显著性
...
BP_MAX_BAD_SIGNAL_COUNT = 8         # 连续异常血压输出的最大容忍次数，超过则强制重捕获

FREQ_SCALE_60G_TO_24G = 24.0 / 60.0 # 60GHz 雷达到 24GHz 的相位折算系数 (适配现有网络)
PHASE_RANGE_MIN_BP = 0.001          # 血压模式最小有效相位极差 (高频信号更微弱)

RADAR_MAX_RANGE_BINS = 32           # 雷达硬件单帧输出的最大 Range Bin 数量
BP_TRACKER_ALPHA = 0.85             # Alpha-Beta 追踪器位置校正增益
BP_TRACKER_BETA = 0.5               # Alpha-Beta 追踪器速度校正增益
BP_EMA_ALPHA = 0.30                  # 血压时序输出的指数移动平均更新权重
...
MIN_VALID_RANGE_BIN = 6             # 近场天线耦合杂波盲区截断索引 (约 15.6cm 内过滤)
